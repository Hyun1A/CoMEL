...
def five_scores(bag_labels, bag_probs, sub_typing=False, return_pred_patch=False, num_classes=2, labels=[0,1]):
...
def five_scores_and_seg_scores(bag_labels, bag_probs, bag_labels_patch, bag_probs_patch, sub_typing=False, return_pred_patch=False, num_classes=2, labels=[0,1],th_patch=None):
    ...
    try:
        auc_value = roc_auc_score(bag_labels_one_hot, bag_probs, multi_class='ovo', average='macro')
    except:
        breakpoint()
    ...
    if th_patch == None:
        _, _, threshold_optimal_patch = optimal_thresh(fpr_patch, tpr_patch, threshold_patch)
        threshold_optimal_patch = threshold_optimal_patch.item()
    else:
        threshold_optimal_patch = th_patch
    ...
    if return_pred_patch:
        return accuracy, auc_value, precision, recall, fscore, count_tumor_slide, \
                mioU, dice, acc_patch, auc_patch, fscore_patch, threshold_optimal_patch, bag_predictions_patch
    else:
        return accuracy, auc_value, precision, recall, fscore, count_tumor_slide, \
                mioU, dice, acc_patch, auc_patch, fscore_patch, threshold_optimal_patch
