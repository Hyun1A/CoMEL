...
class CDATMIL(nn.Module):
    ...
    def expand_orthogonal_lora(self):
        for idx, (full_name, module) in enumerate(self.update_modules.items()):
            ...
            down_weight = lora_module.down.weight.data

            if len(module.lora_layers) > 0:
                down_weight_ortho = None
                for layer in module.lora_layers:
                    down_ortho_mat = layer.down.weight
                    down_ortho_space = down_ortho_mat.T @ down_ortho_mat

                    if down_weight_ortho == None:
                        down_weight_ortho = down_weight @ down_ortho_space
                    else:
                        down_weight_ortho += down_weight @ down_ortho_space

                    down_weight = down_weight - down_weight_ortho

            down_weight = down_weight / down_weight.norm(dim=1, keepdim=True)

            lora_module.down.weight.data = down_weight.data
