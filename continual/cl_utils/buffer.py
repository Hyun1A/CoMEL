...
class Patch_Buffer:
    ...
    def __init__(self, buffer_size, device, n_tasks=None, mode='reservoir'):
        ...
        if mode == 'ring':
            ...
            self.task_number = n_tasks
            self.buffer_portion_size = buffer_size // n_tasks
        self.attributes = ['examples', 'labels', 'patch_labels', 'logits', 'task_labels']
    ...
    def init_tensors(self, examples: torch.Tensor, labels: torch.Tensor, patch_labels: torch.Tensor,
                     logits: torch.Tensor, task_labels: torch.Tensor) -> None:
        """
        Initializes just the required tensors.
        :param examples: tensor containing the images
    ...
    def add_data(self, examples, labels=None, patch_labels=None, logits=None, task_labels=None):
        """
        Adds the data to the memory buffer according to the reservoir strategy.
        :param examples: tensor containing the images
        :param labels: tensor containing the labels
        ...
        examples = [i.to(self.device) for i in examples]
        if not hasattr(self, 'examples'):
            self.init_tensors(examples, labels, patch_labels, logits, task_labels)
        ...
        if index >= 0:
            if index != self.num_seen_examples:
                ...
                if patch_labels is not None:
                    self.patch_labels[index] = patch_labels.to(self.device)
                ...
                if patch_labels is not None:
                    self.patch_labels.append(patch_labels.to(self.device))
