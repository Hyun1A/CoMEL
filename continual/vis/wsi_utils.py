...
import os
import pdb
from vis.util_classes import Mosaic_Canvas
...
def DrawMapFromCoords(canvas, wsi_object, coords, patch_size, vis_level, indices=None, draw_grid=True, unmatched_factor=1):
    downsamples = int( wsi_object.wsi.level_downsamples[vis_level] * unmatched_factor )
    if indices is None:
        indices = np.arange(len(coords))
    total = len(indices)
...
def StitchCoords_online(coords, wsi_object, label_patch, prediction_patch, \
                    downscale=16, draw_grid=False, bg_color=(0,0,0), alpha=-1, patch_size=512, patch_level=0):
    wsi = wsi_object.getOpenSlide()
    w, h = wsi.level_dimensions[0]
    # print('original size: {} x {}'.format(w, h))
    ...
    wsi_downsamples = wsi.level_downsamples[vis_level]
    unmatched_factor = int(downscale // wsi_downsamples)
    # print("unmatched_factor:", unmatched_factor)
    print('downscaled size for stiching: {} x {}'.format(w//int(unmatched_factor), h//int(unmatched_factor)))
    ...
    img_size = heatmap.size
    
    
    img_heatmap = np.array(heatmap)
    img_heatmap = DrawMapFromCoords(img_heatmap, wsi_object, coords, patch_size, vis_level, indices=None, draw_grid=draw_grid)
    img_heatmap = img_heatmap.resize((img_size[0]//unmatched_factor, img_size[1]//unmatched_factor))
    
    label_heatmap = np.array(heatmap)
    label_heatmap = DrawHueFromCoords(label_heatmap, wsi_object, label_patch, coords, patch_size, vis_level,\
                            indices=None, draw_grid=draw_grid, hue=[0,255,0])
    label_heatmap = label_heatmap.resize((img_size[0]//unmatched_factor, img_size[1]//unmatched_factor))
    overlap_label = np.array(img_heatmap).astype(np.float32)
    where_label = ( np.array(label_heatmap).astype(np.float32).sum(axis=2) != 0 )
    overlap_label[where_label] = 0.25*overlap_label[where_label] + 0.75*np.array(label_heatmap).astype(np.float32)[where_label]
    
    pred_heatmap = np.array(heatmap)
    pred_heatmap = DrawHueFromCoords(pred_heatmap, wsi_object, prediction_patch, coords, patch_size, vis_level,\
                            indices=None, draw_grid=draw_grid, hue=[0,0,255])
    pred_heatmap = pred_heatmap.resize((img_size[0]//unmatched_factor, img_size[1]//unmatched_factor))
    overlap_pred = np.array(img_heatmap).astype(np.float32)
    where_pred = ( np.array(pred_heatmap).astype(np.float32).sum(axis=2) != 0 )
    overlap_pred[where_pred] = 0.25*overlap_pred[where_pred] + 0.75*np.array(pred_heatmap).astype(np.float32)[where_pred]

    concat_heatmap = np.concatenate([overlap_label, overlap_pred], axis=1)


    label_heatmap = Image.fromarray(overlap_label.astype(np.uint8))        
    pred_heatmap = Image.fromarray(overlap_pred.astype(np.uint8))
    concat_heatmap = Image.fromarray(concat_heatmap.astype(np.uint8))

    return img_heatmap, label_heatmap, pred_heatmap, concat_heatmap

def DrawHueFromCoords(canvas, wsi_object, label, coords, patch_size, vis_level, indices=None, draw_grid=True, hue=[255,255,255]):
    ...
    for idx in range(total):        
        patch_id = indices[idx]
        coord = coords[patch_id]        
        patch = label[idx]*( np.ones((patch_size[0], patch_size[1], 3)) * np.array(hue) ).astype(np.uint8)
        coord = np.ceil(coord / downsamples).astype(np.int32)
        canvas_crop_shape = canvas[coord[1]:coord[1]+patch_size[1], coord[0]:coord[0]+patch_size[0], :3].shape[:2]
        canvas[coord[1]:coord[1]+patch_size[1], coord[0]:coord[0]+patch_size[0], :3] = patch[:canvas_crop_shape[0], :canvas_crop_shape[1], :]
        if draw_grid:
