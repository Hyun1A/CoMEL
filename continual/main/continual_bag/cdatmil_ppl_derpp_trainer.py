...
class CDATMIL_PPL_DERPP_Trainer(CDATMIL_PPL_FT_Trainer):
    ...
    def train_loop(self, args,model,loader,optimizer,device,amp_autocast,criterion,loss_scaler,scheduler,k,epoch, criterion_reg=None):
        ...
        for i, data in enumerate(loader):
            ...
            if self.current_task > 0:                
                buf_bag, buf_label, buf_logits = self.buffer.get_data(bs)
            ...
            n_slides, quality, pred, val, train_loss, logit_loss, patch_logit_loss, cls_loss, keep_num, patch_num, train_acc, train_logits = \
                    self.observe(n_slides, quality, bag, amp_autocast, args, model, criterion, batch_size, label, patch_labels, epoch, train_acc, i)

            if self.current_task > 0:
                _, _, buf_pred, buf_val, buf_train_loss, buf_logit_loss, buf_patch_logit_loss, buf_cls_loss, buf_keep_num, buf_patch_num, _, _ = \
                        self.observe(n_slides, quality, buf_bag, amp_autocast, args, model, criterion, batch_size, buf_label, patch_labels, epoch, train_acc, i,\
            ...
            #############################################
            ################# ER Buffer #################
            self.buffer.add_data(examples=data[0], labels=data[1], logits=train_logits)
            ################# ER Buffer #################
            #############################################
    ...
    def observe(self, n_slides, quality, bag, amp_autocast, args, model, criterion, batch_size, label, patch_labels, epoch, train_acc, i, **kwargs):
        ...
        with amp_autocast():
            ...
            if len(kwargs) > 0:
                buf_logits = kwargs["buf_logits"]

                if buf_logits.size(1) != train_logits.size(1):
                    len_label = min(buf_logits.size(1), train_logits.size(1))

                    
                    derpp_loss = self.args.der_alpha * F.mse_loss(buf_logits[:,:len_label], train_logits[:,:len_label])
                else:
                    derpp_loss = self.args.der_alpha * F.mse_loss(buf_logits, train_logits)

                train_loss += derpp_loss
        ...
        return n_slides, quality, pred, val, train_loss, logit_loss, patch_logit_loss, cls_loss, keep_num, patch_num, train_acc, train_logits