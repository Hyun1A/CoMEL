...
class CDATMIL_PPL_EWC_Trainer(CDATMIL_PPL_FT_Trainer):
    ...
    def train_loop(self, args,model,loader,optimizer,device,amp_autocast,criterion,loss_scaler,scheduler,k,epoch, criterion_reg=None):
        ...
        for i, data in enumerate(loader):
            ...
            with amp_autocast():
                ...
                train_loss += self.args.e_lambda * penalty
    ...
    def after_task(self):
        ...
        for j, data in enumerate(self.train_loader):
            ...
            logits, _, _, _ = self.model(inputs,return_attn=True,no_norm=False,return_inst=True)    
            logit_loss = self.criterion(logits.view(1,-1),labels)

            exp_cond_prob = torch.mean(torch.exp(logit_loss.detach().clone()))

            logit_loss.backward()

            fish += exp_cond_prob * self.get_grads() ** 2

        fish /= (len(self.train_loader) * bs)

        if self.fish is None:
            self.fish = fish
        else:
            self.fish *= self.args.e_gamma
            self.fish += fish
    ...
    def get_params(self):
        ...
        """
        params = []
        for n, pp in list(self.model.named_parameters()):
