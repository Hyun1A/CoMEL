...
class CDATMIL_PPL_LwF_Trainer(CDATMIL_PPL_FT_Trainer):
    def __init__(self, args, k, train_p, train_l, test_p, test_l,val_p,val_l,dataset_root_organ_list):
        ...
        self.current_task = 0
        self.cpt = len(self.labels)
        self.cl_warm_epochs = 20
    ...
    def before_task(self):
        ...
        if self.current_task > 0:
            ...
            opt = torch.optim.Adam(self.model.classifier.parameters(), lr=self.args.lr)

            lwf_loader = DataLoader(self.train_set, batch_size=self.args.batch_size, shuffle=False, num_workers=self.args.num_workers)

            logits = []
            for i, data in enumerate(lwf_loader):
                if isinstance(data[0],(list,tuple)):
                    for i in range(len(data[0])):
                        data[0][i] = data[0][i].to(self.device)
                    bag=data[0]
                    batch_size=data[0][0].size(0)
                else:
                    bag=data[0].to(self.device)  # b*n*1024
                    batch_size=bag.size(0)
                

                label=data[1].to(self.device)
                patch_labels = data[2].squeeze(0).squeeze(-1).to(self.device)        
                ...
                if self.args.model in ('cdatmil'):
                    with torch.no_grad():
                        train_logits, train_patch_attn, feat_bag, feat_inst = self.model(bag,return_attn=True,no_norm=False,return_inst=True)    
                        train_patch_attn = train_patch_attn.permute(1,0)

                    logits.append(train_logits)
                
                ############### Model forward #################
                ###############################################

            setattr(self.train_loader.dataset, 'logits', torch.cat(logits))
    ...
    def train_loop(self, args,model,loader,optimizer,device,amp_autocast,criterion,loss_scaler,scheduler,k,epoch, criterion_reg=None):
        ...
        for i, data in enumerate(loader):
            ...
            with amp_autocast():
                ...
                ################### LwF loss ####################
                if self.current_task > 0:
                    frozen_logits = self.train_loader.dataset.logits[data[5]]
                    mask = self.labels[0]
                    train_loss += self.args.lwf_alpha * modified_kl_div(smooth(self.soft(frozen_logits[:, :mask]).to(self.device), 2, 1), smooth(self.soft(train_logits[:, :mask]), 2, 1))
