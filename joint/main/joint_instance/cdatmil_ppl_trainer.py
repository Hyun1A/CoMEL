...
class CDATMIL_PPL_Trainer(ABMIL_Trainer):
            ...
            with amp_autocast():
                ...
                if args.loss == 'ce':
                    ...
                    if ("paip" in self.args.datasets) or ("cm16" in self.args.datasets):
                        ...
                        feat_bag_avg = 0.5*(feat_bag + feat_bag_rev)

                        feat_inst_norm = (feat_inst-feat_bag_avg) / (feat_inst-feat_bag_avg).norm(dim=1, keepdim=True)
                        feat_bag_norm = (feat_bag-feat_bag_avg) / (feat_bag-feat_bag_avg).norm(dim=1, keepdim=True)
                        feat_bag_rev_norm = (feat_bag_rev-feat_bag_avg) / (feat_bag_rev-feat_bag_avg).norm(dim=1, keepdim=True)
                        ...
                        val_sorted, inds_sorted = disc_val.sort(descending=True) # [:5].detach().cpu()
                        
                        if is_tumor:
                            sim_loss = feat_bag/feat_bag.norm(dim=1, keepdim=True) @ (feat_bag_rev/feat_bag_rev.norm(dim=1, keepdim=True)).T
                        else:
                            sim_loss = feat_bag/feat_bag.norm(dim=1, keepdim=True) @ (feat_bag_rev/feat_bag_rev.norm(dim=1, keepdim=True)).T
                            # print(f"[{label.item()%2}]")
                            sim_loss = torch.ones_like(logit_loss).to(feat_bag.device)

                        
                        val_idx = (val>self.tau1)*(disc_val > self.tau2)*(disc_pred == 1)
                        ...
                        if ((is_tumor and is_correct)) and (patch_log_softmax.size(0) > 0):
                            patch_logit_loss = F.nll_loss(patch_log_softmax, sel_disc_pred) 
                            quantity = (len(sel_disc_pred)/len(disc_pred[disc_pred == 1]))
                            qual = ((sel_disc_pred==patch_labels[val_idx]%2).sum()/len(sel_disc_pred))

                            print(f"[{label.item()%2}], sim: {sim_loss.item():7.5f}, patch_loss: {patch_logit_loss.item():7.5f}, {val_sorted[:2].detach().cpu()}, pl quantity: {quantity:7.5f}, pl_quality: {qual:7.5f}")
                            patch_logit_loss = quantity * patch_logit_loss                    
                        else:
                            patch_logit_loss = torch.zeros_like(logit_loss).to(feat_bag.device)
                ...
                train_loss = args.cls_alpha * logit_loss +  cls_loss*args.aux_alpha + 0.1*sim_loss
