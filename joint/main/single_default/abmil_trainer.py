...
from modules import attmil,clam,dsmil,transmil,mean_max,rrt, cdatmil
import losses
...
class ABMIL_Trainer:
    ...
    def set_model(self, args):
        ...
        if args.model == 'rrtmil':
            ...
            model = cdatmil.CDATMIL(**model_params).to(self.device)




        elif args.model == 'agentmil':
            model_params = {
                'input_dim': args.input_dim,
                'n_classes': args.n_classes,
                'dropout': args.dropout,
            ...
            model = agentmil.AgentMIL(**model_params).to(self.device)
    ...
    def train(self):      
        ...
        for epoch in range(epoch_start, self.args.num_epoch):
            ...
            if not ((epoch)%self.args.val_interval==0 or epoch == self.args.num_epoch-1):
    ...
    def val_loop(self, args,model,loader,device,criterion,early_stopping,epoch,labels=[0,1]):
        ...
        accuracy, auc_value, precision, recall, fscore, count_tumor_slide, miou, dice, acc_patch, auc1_patch, fscore_patch, th_patch, bag_predictions_patch = \
                                    five_scores_and_seg_scores(bag_labels, bag_logit, bag_labels_patch, bag_logit_patch, subtyping, return_pred_patch=True, labels=labels)    
