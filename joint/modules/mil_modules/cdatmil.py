...
class CDATMIL(nn.Module):
    def __init__(self, input_dim=1024,mlp_dim=512,act='relu',n_classes=2,dropout=0.25,pos_pos=0,pos='none',\
        ...
        self.predictor = nn.Linear(self.mlp_dim,n_classes)
    ...
    def forward(self, x, return_attn=False,no_norm=False, return_inst=False):
        x, x_rep, a = self.forward_aggregator(x=x, 
                                              return_attn=return_attn,
                                              no_norm=no_norm)

        # prediction
        logits = self.predictor(x_rep)
        ...
        if not return_inst:
            ...
            if return_attn:
                return logits,a,x_rep,x
            else:
                return logits,x_rep,x
